    ...
    async def color(this, ctx: discord.ApplicationContext) -> None:
        ...
        with open("output.png", "rb") as f:  # noqa: ASYNC230
            await ctx.response.send_message(
                f"**#{format(color[0], '02x')}{format(color[1], '02x')}{format(color[2], '02x')}** is your random unique color!"
                if color not in this.colorSet
                else f"**#{format(color[0], '02x')}{format(color[1], '02x')}{format(color[2], '02x')}**\
        ...
        async with aiofiles.open("colorlist.bin", "ab") as f:
            await f.write(fileOutput)
